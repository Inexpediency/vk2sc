...
class VkParser:
    ...
    def parsing(playlist_link):
        ...
        if (music_list == False):
            exit('I can\'t parse ur music bro :(')
        
        playlist_title = music_list.get('title')

        playlist = music_list.get('list')

        music_labels = []

        for mus in playlist:
            mus_title = mus[3] + ' ' + mus[4]
            music_labels.append(mus_title)
